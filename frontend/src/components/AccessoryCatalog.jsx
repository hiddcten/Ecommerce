...
const AccessoryCatalog = () => {
  ...
  const handleSort = (criteria) => {
    ...
    switch (criteria) {
      ...
      case "price-high":
        ...
        break;
      case "rating":
        sortedAccessories.sort((a, b) => b.averageRating - a.averageRating);
        break;
      default:
  ...
  const filteredAccessories = accessories.filter(accessory => {
    const matchesPriceFilter = filterPrice ? accessory.price <= parseFloat(filterPrice) : true;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"> {/* Cập nhật khoảng cách */}
          {getCurrentPageItems().map((accessory) => (
            <div
              ...
              role="article"
              onClick={() => navigate(`/accessory/${accessory.slug}`)}
            >
              <img
